...
{
    private static readonly string[] _ignore = new string[] { "forge", "minecraft", "fabricloader", "java", "fabric", "fabric-api", "fabric-lifecycle-events-v1", "fabric-command-api-v2", "fabric-api-base", "fabric-api" };

    public static TomlTable? GetMetaToml(ZipArchive zip, string name) //read metadata from all mods in folder .minecraft/mods
    {
        var meta = zip.Entries.FirstOrDefault(e => e.FullName.Equals("META-INF/mods.toml", StringComparison.InvariantCultureIgnoreCase));
    ...
    {
        var meta = zip.Entries.FirstOrDefault(e => e.FullName.Equals("fabric.mod.json", StringComparison.InvariantCultureIgnoreCase));
        if (meta == null) return null;

        var text = new string(new StreamReader(meta.Open(), Encoding.UTF8).ReadToEnd()) ?? throw new MetaDataException(name);

        var json = JsonConvert.DeserializeObject<MetaJson>(text);
        if (json == null) throw new MetaDataException(meta.FullName);
    ...
    public static IEnumerable<string> GetDependencies(TomlTable model)
    {
        ...
        if (!model.ContainsKey("dependencies")) return found;
        ...
        return found.Except(_ignore).ToList();
    }
    public static string GetId(TomlTable model, string name)
    {
        ...
        if (found.Count < 1) throw new MetaDataException(model, name);
        return found.First();
    }
    public static IEnumerable<string> GetDependencies(MetaJson json, string name)
    {
        if (json.Depends == null) throw new MetaDataException(json, name);
        var dependencies = json.Depends.Keys.ToList();
        return dependencies.Except(_ignore.Concat(new[] { name })).ToList();
    }
    public static string GetId(MetaJson json, string name)
    {
        if (json.Id == null) throw new MetaDataException(json, name);
        return json.Id;
    }
}
